...
export class DataSyncService {
  ...
  public async updateVehicleData(vehicleData: IVehicleData): Promise<void> {
    if (vehicleData) {
      ...
      console.log(`${vehicleData.display_name} is currently ${state} : ${vehicleData.charge_state.charging_state} : ${vehicleData.drive_state.shift_state || 'parked'} `);
  ...
  private async appendChargeState(session: IChargeSession, vehicleData: IVehicleData): Promise<any> {
    ...
    const state = <IChargeState>await ChargeState.create({
      ...
      is_climate_on: vehicleData.climate_state.is_climate_on || false,
      is_front_defroster_on: vehicleData.climate_state.is_front_defroster_on || false,
      is_preconditioning: vehicleData.climate_state.is_preconditioning || false,
      is_rear_defroster_on: vehicleData.climate_state.is_rear_defroster_on || false,
      ...
      is_user_present: vehicleData.vehicle_state.is_user_present || false,
    ...
    session.end_date = new Date(state.timestamp);
  ...
  private async appendDriveState(session: IDriveSession, vehicleData: IVehicleData): Promise<any> {
    ...
    session.last = state;
    session.end_date = new Date(session.last.timestamp);
